...
export function AddCollectionMobile({
  fileImg,
  loadImg,
  deleteImg,
  changeStyleSelect,
  ...
  const headerTemplate = () => {
    return (
      <div className="Add-collection__mobile-header">
        ...
        {textInputsShow ? (
          <button
            onClick={() => changeAddCollectionInputs(false)}
            className="Add-collection__mobile-next-button"
          >
            <FontAwesomeIcon icon="chevron-left" color="#000" />
            <span className="Add-collection__mobile-back-button-text">
              назад
            </span>
          </button>
        ) : (
          <button
            disabled={!fileImg}
            onClick={() => changeAddCollectionInputs(true)}
            className="Add-collection__mobile-next-button"
          >
            <span className="Add-collection__mobile-next-button-text">
              далее
            </span>
  ...
  return (
    <div className="Add-collection__mobile">
      {headerTemplate()}
      {textInputsShow ? (
        <section>{inputTemplate()}</section>
      ) : (
        <section>{imageTemplate()}</section>
      )}
    </div>
  );
}
