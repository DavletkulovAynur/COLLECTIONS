...
class SubscribeController {
  async gelAllSubscribeUser(req, res) {
    try {
      ...
      const onlySubscribers = users.filter((item) =>
        user[0].subscribers.includes(String(item._id))
      );
      const onlySubscriptions = users.filter((item) =>
        user[0].subscriptions.includes(String(item._id))
      );
  ...
  async subscribeOnUser(req, res) {
    try {
      ...
      await USER_MODEL.updateMany(
        { _id: req.user.id },
        { $addToSet: { subscriptions: subscribeUserId } }
      );
  ...
  async unSubscribeUser(req, res) {
    try {
      ...
      await USER_MODEL.updateMany(
        { _id: req.user.id },
        { $pull: { subscriptions: subscribeUserId } }
      );
      // вернуть подписки юзера
      res.status(201).json({ message: "Успешно" });
    } catch (e) {
